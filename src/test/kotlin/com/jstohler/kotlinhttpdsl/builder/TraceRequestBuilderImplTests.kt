...
class TraceRequestBuilderImplTests : AbstractMockServer() {

    @Test
    fun `build TRACE with body Test`() {
        val request = RequestBuilderImpl.build {
            trace("https://jstohler.com") {
                "{'some-json': 'body'}"
            }
        ...
        assertThat(request.httpVerb, equalTo(HttpVerb.TRACE))
    ...
    @Test
    fun `build TRACE with no body Test`() {
        val request = RequestBuilderImpl.build {
            trace("https://jstohler.com")
        }
    ...
    @Test
    fun `build TRACE with header and no body Test`() {
        val request = RequestBuilderImpl.build {
            trace("https://jstohler.com")
            header { "Authorization: Bearer some_token" }
        }
    ...
    @Test
    fun `build TRACE with multiple header blocks Test`() {
        val request = RequestBuilderImpl.build {
            trace("https://jstohler.com") {
                "{'some-json': 'body'}"
            }
    ...
    @Test
    fun `build TRACE with headers collection blocks Test`() {
        val request = RequestBuilderImpl.build {
            trace("https://jstohler.com") {
                "{'some-json': 'body'}"
            }
    ...
    @Test
    fun `build TRACE with no value Test`() {
        assertThrows<IllegalArgumentException> { RequestBuilderImpl.build {} }
    }

    @Test
    fun `build TRACE with an empty address and body Test`() {
        assertThrows<IllegalArgumentException> { RequestBuilderImpl.build { trace("") { "something" } } }
    }

    @Test
    fun `request TRACE Test`() {
        ...
        val actual = RequestBuilderImpl.request {
            trace("http://localhost:1111") { "some-input-body" }
        }
        ...
        assertThat(actual.request.method(), equalTo("TRACE"))
    ...
    @Test
    fun `request TRACE without body Test`() {
    ...
    @Test
    fun `request TRACE with body and header Test`() {
    ...
    @Test
    fun `request TRACE with Body and multiple header Test`() {
    ...
    @Test
    fun `request TRACE with Body and headers Test`() {
