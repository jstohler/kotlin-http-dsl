...
class GetRequestBuilderImplTests : AbstractMockServer() {
    ...
    @Test
    fun `build GET with no value Test`() {
        assertThrows<IllegalArgumentException> { RequestBuilderImpl.build {} }
    }

    @Test
    fun `build GET with an empty address and body Test`() {
        assertThrows<IllegalArgumentException> { RequestBuilderImpl.build { get("") { "something" } } }
    }

    @Test
    fun `request GET Test`() {

        // Mock Response
        val mockRequest = request().withMethod("GET")
        val mockResponse = response().withStatusCode(200).withBody("hello-world")

        // Mocks
        server.`when`(mockRequest, Times.exactly(1)).respond(mockResponse)
    ...
    @Test
    fun `request GET with body Test`() {
        ...
        // SUT
        val actual = RequestBuilderImpl.request {
            get("http://localhost:1111") { "some-input-body" }
        }
    ...
    @Test
    fun `request GET with body and header Test`() {

        // Mock Response
        val mockRequest = request().withMethod("GET").withHeader("Authorization: Bearer some-token")
        val mockResponse = response().withStatusCode(200).withBody("hello-world")
    ...
    @Test
    fun `request GET with Body and multiple header Test`() {
    ...
    @Test
    fun `request GET with Body and headers Test`() {
